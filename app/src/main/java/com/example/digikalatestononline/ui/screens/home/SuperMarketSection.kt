...
@Composable
fun SuperMarketSection(navController: NavController, viewModel: HomeViewModel = hiltViewModel()) {
    var superMarketList by remember { mutableStateOf<List<AmazingItem>>(emptyList()) }
    var loading by remember { mutableStateOf(false) }

    val amazingItemResult by viewModel.superMarketItem.collectAsState()

    when (amazingItemResult) {
        is NetworkResult.Success -> {
            superMarketList = amazingItemResult.data ?: emptyList()
            loading = false
        }

        is NetworkResult.Error -> {
            Log.e("6191", "amazingItem Api Error :${amazingItemResult.message}")
            loading = false
        }
